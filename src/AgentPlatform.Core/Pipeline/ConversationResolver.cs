...
public sealed class ConversationResolver(IConversationRepository repo)
{
    ...
    public async Task<ConversationContext> ResolveAsync(InputMessage msg, CancellationToken ct)
    {
        var active = await repo.GetActiveAsync(msg.UserId, msg.ChannelId, ct);
        ...
        if (IsIncognitoCommand(msg.Text))
        {
            ...
            var inc = await repo.CreateAsync(msg.UserId, msg.GroupId, msg.ChannelId, isIncognito: true, ct);
        ...
        if (shouldStartNew)
            active = await repo.CreateAsync(msg.UserId, msg.GroupId, msg.ChannelId, isIncognito: false, ct);
